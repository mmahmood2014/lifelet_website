      ...
      if (($this->pages) > 1) {
        ...
        for ($j = 1; $j < ($this->pages + 1); $j++) {
          if ($this->page == $j) {
            $prev="";
			if($j==1){
				$prev='<span class="prev_page selected">&laquo; Back</span>';	
			}
			$links[] = ' 
			'.$prev.'
			<a class="selected links_page">'.$j.'</a> 
			'; // If we are on the same page as the current item
          } else {
            $links[] = ' <a class="links_page" href="?page='.$j.$queryUrl.'">'.$j.'</a> '; // add the link to the array
          }
        }
        ...
        if ($this->page != 1) {
          ...
          $plinks[] = ' <a class="prev_page" href="?page='.($this->page - 1).$queryUrl.'">&laquo; Back</a> ';
        }

        // Assign the 'next page' if we are not on the last page
        if ($this->page < $this->pages) {
          $slinks[] = ' <a  class="next_page" href="?page='.($this->page + 1).$queryUrl.'"> Next &raquo; </a> ';
          if ($this->_showFirstAndLast) {
