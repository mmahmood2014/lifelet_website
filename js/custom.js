...
function addBucket(pid, vid){
	...
	$.ajax({
		...
		success:function(response){
			...
			total_plus_spacer=JSONObject.total_plus_spacer;
			...
			$('#subTotal').html('$'+total);
			$('#total_plus_spacer').html('$'+total_plus_spacer);
...
function addToCart(){
	...
	if(total_life_exp==count_life_exp){
		...
		if(count_life_exp > total_life_exp){
			warning_more_items=$('#warning_more_items').html();
			alert(warning_more_items);	
			return false;
		}
		else{
...
function changeState(option){
	/* if discount code is set to 100% and gran_total is 0 */
	...
	/*$.ajax({
		...
		success:function(response){
			...
			$('#tax').val(tax);
			getShipping(tax); //comment hatana hy bad me
...
function getShipping(stateCode){
	
	
	subTotal=$('#sub_total').val();
	target_type=$('#target_type').val();
	var dataString='subTotal='+subTotal+'&stateCode='+ stateCode+'&target_type='+ target_type;
	$.ajax({
		type:"POST",
		...
		success:function(response){
			...
			tax_percent=JSONObject.tax_percent;
			tax_percent_format=JSONObject.tax_percent_format;
			tax_value=JSONObject.tax_value;
			tax_value_format=JSONObject.tax_value_format;
			...
			$('#tax').val(tax_value);
			$('#tax_check').html(tax_value_format);
...
function bsAlert(){
	...
	if(parseInt(count_life_exp)==parseInt(total_life_exp)){
		...
		/* get exactly_warning message from hidden div */
		var warning_exactly_hidden=$('#warning_exactly_hidden').html();
		
		var html='<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>' + warning_exactly_hidden;
		$('#popup_msg').html(html);
